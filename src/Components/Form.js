...
const Form = ({editFormVisibility, editTodo, cancelUpdate}) => {
  ...
  return (
   ...
   <div className='xl:text-[15px] text-[10px] text-gray-500 mb-14'>Add A Task</div>
...
<IoTodayOutline className='text-[25px]'/>
    ...
    <div className="flex justify-end w-full gap-2">
        <input className=" rounded-md px-5 lg:w-60 w-40" type="text"  required 
        value={todoValue} onChange={(e)=>setTodoValue(e.target.value)} />
        <button className="bg-[#11560342] py-1 text-green-800 px-2 xl:text-[15px] text-[10px] rounded-md " type="submit">Add Task</button>
    </div>
    <div>
    </div></div>
